...
def codec_page(result: model.RunResult, codec: str, charts: str) -> str:
    """Docs fragment for one codec: its rows, then its sweep charts as MyST
    image directives in Furo's only-light/only-dark pairing -- a <picture>
    would follow the OS, not the site's theme toggle."""
    lines = [table(result, codecs=(codec,)).rstrip("\n"), ""]

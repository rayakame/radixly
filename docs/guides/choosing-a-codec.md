...
two to four bytes as UTF-8 and base64 or raw binary will beat all of them.
That is not a weakness to work around, it is the wrong tool.
...
base32768 and base2048 about a quarter of the time, hexagram about a third)
or with an error that depends on where the cut landed. If that matters and uro14 does not fit, carry a
...
base65536 the one a channel might reject for leaving the BMP.

...
const char radixly_base32768_encode_doc[] =
    PyDoc_STR("base32768_encode($module, data, /)\n"
              ...
              "\n"
              "Every character carries 15 payload bits, a final short character 7,\n"
              "so ``n`` input bytes become ``ceil(8 * n / 15)`` characters -- all of\n"
              "them BMP code points from qntm's base32768 alphabet.\n"
              "\n"
              "Parameters\n"
              "----------\n"
              "data\n"
